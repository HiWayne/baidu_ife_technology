...
class FootballPlayer {
    ...
    getFootballPosition(r, s) {
        ...
        if (Math.sqrt((posX - football.posX) * (posX - football.posX) + (posY - football.posY) * (posY - football.posY)) <= footballfield.meterToPx(1)) {
            football.stop()
            this.kickBall(this.caculateRadian(r), this.caculateStrength(s))
            return
        }
    }
    //根据球员当前坐标渲染球员
        ...
        return new Promise((resolve) => {
            ...
            let animate = () => {
                ...
                if (!runParam || !runParam.onlyRun) {
                    //跟踪足球位置
                    this.getFootballPosition(runParam.radian, runParam.strength)
                }
                posX = this.posX
                posY = this.posY
