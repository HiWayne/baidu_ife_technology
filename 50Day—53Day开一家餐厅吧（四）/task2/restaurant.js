...
class Restaurant {
    constructor(seats, money) {
        ...
        this.money = null
        //监听属性money
        Restaurant.observeProperty(this, "money", this.money, updataMoney, this)
        function updataMoney (value) {
            let moneySpan = document.getElementById("restaurantMoney")
            moneySpan.innerHTML = value
        }
        this.money = money
    ...
    */
    ...
    order(customer) {
        ...
        if (!waiterList) {
            throw new Error("餐厅没有服务员")
        }
        let len = this.staff.waiter.length
        if (len) {
            for (let i = 0; i < len; i++) {
                //如果有空闲的服务员
                if (waiterList[i].atLeisure) {
                    let orderDishes = customer.order()
                    waiterList[i].work(orderDishes, customer)
                    return
                }
            }
            let suspension = this.customerSuspension
            suspension.push(customer)
        }
    }
    //做菜方法
    cook(method) {
    ...
    static observeProperty(obj, key, value, fn, target) {
        target = target || null
...
//现在有两个问题：为什么服务员总是一去服务。厨师splice数组cooking时，index是变动的。
...
class Waiter extends Staff {
    ...
    ownWork(parameter, customer) {
        let _this = this
        this.atLeisure = false
        //服务员点菜
        if (Array.isArray(parameter) && customer) {
            setTimeout(work, 500)
            //work函数没有调用对象，或者说调用对象是window，所以this是window的this，那么就要使用_this
            function work() {
                console.log(`${_this.name} 移动到顾客处`)
                console.log(`${customer.name} 点菜`)
                console.log(parameter)
                console.log(`${_this.name} 把菜放在厨房白板`)
                for (let i = 0; i < parameter.length; i++) {
                    let kind = parameter[i].name
                    ...
                    restaurant.dishesList.push(parameter[i])
                }
                if (_this.fired) {
                ...
                let len1 = restaurant.doneDishes.length
                let len2 = restaurant.customerSuspension.length
                //如果有做好的菜待上
                if (len1) {
        ...
        //服务员上菜
            ...
            function work() {
                ...
                _this.atLeisure = true
                let len1 = restaurant.customerSuspension.length
                let len2 = restaurant.doneDishes.length
                //如果有需要点菜的被暂时挂起的顾客
                if (len1) {
                    let customer = restaurant.customerSuspension.shift()
                    restaurant.order(customer)
    ...
    eat(method) {
        ...
        if (len) {
            ...
            function enjoy() {
                ...
                dish.state = "已吃完"
                console.log(`${_this.name} eat ${dish.name}`)
